...
This project provides a Docker container for [Krusader](https://krusader.org).
...
services:
  krusader:
    image: jlesage/krusader
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/krusader:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-krusader/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/krusader/tags
...
container, it would proxy all HTTP requests for `krusader.domain.tld` to
...
server {
	...
	server_name krusader.domain.tld;

	location / {
		proxy_pass http://docker-krusader;
	}
...
server {
	...
	location = /krusader {return 301 $scheme://$http_host/krusader/;}
	location /krusader/ {
		proxy_pass http://docker-krusader/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location ~ ^/krusader/(websockify(-.*)?) {
                        proxy_pass http://docker-krusader/$1;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of Krusader integrated into Docker image. |
...
Any desktop notification generated by Krusader is forwarded to the
